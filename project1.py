...
def hinge_loss(feature_matrix, labels, theta, theta_0):
    ...
    for i in range(num_labels):
        temp = theta.dot(feature_matrix[i])
        temp = 1 - labels[i] * (theta_0 + temp)
        total_hinge_loss += max(0, temp)
    return total_hinge_loss / num_labels
...
def perceptron(feature_matrix, labels, T):
    ...
    theta = np.zeros((feature_matrix[0].size,))
    theta_0 = 0.0
    for j in range(T):
        for i in range(num_labels):
...
def passive_aggressive_single_step_update(feature_vector, label, L, current_theta, current_theta_0):
    ...
    completed.
    """
    temp = hinge_loss(np.array([feature_vector,]), np.array([label,]), current_theta, current_theta_0)
    temp /= (1 + feature_vector.dot(feature_vector))
    coeff = min(1.0/L, temp)
    current_theta += coeff * label * feature_vector
    current_theta_0 += coeff * label
    return (current_theta, current_theta_0)
...
def average_perceptron(feature_matrix, labels, T):
    ...
    theta = np.zeros((feature_matrix[0].size,))
    theta_0 = 0.0
    avg_theta = np.zeros((feature_matrix[0].size,))
    avg_theta_0 = 0.0
    c = 1.0
    for j in range(T):
        for i in range(num_labels):
            if labels[i] * (theta.dot(feature_matrix[i]) + theta_0) <= 0:
                ...
                theta_0 += labels[i]
                avg_theta += c * labels[i] * feature_matrix[i]
                avg_theta_0 += c * labels[i]
            c -= 1.0/(T * num_labels)
...
def average_passive_aggressive(feature_matrix, labels, T, L):
    ...
    for j in range(T):
        for i in range(num_labels):
            temp = hinge_loss(np.array([feature_matrix[i],]), np.array([labels[i],]), theta, theta_0)
            temp /= (1 + feature_matrix[i].dot(feature_matrix[i]))
            coeff = min(1.0/L, temp)
            theta += coeff * labels[i] * feature_matrix[i]
            theta_0 += coeff * labels[i]
            avg_theta += c * coeff * labels[i] * feature_matrix[i]
            avg_theta_0 += c * coeff * labels[i]
            c -= 1.0/(T * num_labels)
    return (avg_theta, avg_theta_0)
...
def classify(feature_matrix, theta, theta_0):
    ...
    for i in range(feature_matrix.shape[0]):
        if feature_matrix[i].dot(theta) + theta_0 > 0:
            temp.append(1)
        else:
            temp.append(-1)
...
def perceptron_accuracy(train_feature_matrix, val_feature_matrix, train_labels, val_labels, T):
    ...
    temp = classify(train_feature_matrix, theta, theta_0)
    temp2 = classify(val_feature_matrix, theta, theta_0)
    return (accuracy(train_labels, temp), accuracy(val_labels, temp2))
...
def extract_words(input_string):
    ...
    input_lower = input_string.lower().split()
    for i in xrange(len(input_lower) - 1):
        input_lower.append(input_lower[i] + ' ' +input_lower[i + 1])
    return input_lower
...
def bag_of_words(texts):
    ...
    occurence = {} # maps word to number of occurence in the texts
    for text in texts:
